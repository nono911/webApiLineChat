...
class FirebaseService {
    ...
    getAllInfo(userID,replyToken) {
	  ...
	  getonListener(userID,replyToken) {
        ...
        db.ref("/users/"+userID).once("value").then(function(snapshot) {
             ...
             console.log("devcie In " + deviceID);
             if (deviceID!="") {
                  var ref = admin.database().ref(deviceID+"/"+date_today());
                //var listener ;
  ...
  /* listener = */  listenerarray.push(ref.limitToLast(1).on('child_added',function(snapshot) {
                        ...
                        admin.database().ref("/device/"+deviceId).once("value").then(function(snapshot) {
...
//                      botsendmessage(userID, deviceID +" " +ddate + "\n" + _car.toString().substring(0,1500));
    ...
    getEnabledInsurance() {
        return new Promise(function (resolve, reject) {
            try {
                var ref = admin.database().ref("863703039069468/"+date_today());
  ...
  //   var runfirst = 0;
     listenerdevicearray=[];
     listenerarray=[];
     admin.database().ref("/device/").once("value").then(function(snapshot) {
       /*
       snapshot.forEach(userSnapshot => {
           nkey = userSnapshot.key;
           btime = userSnapshot.val().btime;
                                    ...
                                    //   var userId = "";
                                      ...
                                      // console.log(`snapshot : ${ snapshot.key }`);
                                       ...
                                       console.log(deviceId+"/"+date_today()+"/"+key);
                                         var letontime = admin.database().ref(deviceId+"/"+date_today()+"/"+key);
